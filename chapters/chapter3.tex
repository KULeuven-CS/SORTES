...
To select suitable hardware \index{hardware} for a real-time embedded system we should first define what suitable means in this case. Suitable hardware is hardware that can satisfy the given (time) constraints and is not too overdimensioned.

\section{Select a maximal resource structure}

\begin{itemize}
	\item Each process executes on a different processor \index{processor} and has all the physical resource it needs.
...
The following are properties of a maximal resource \index{resource} structure.
\begin{itemize}
...
\begin{enumerate}
	\item No process migration \index{process migration} (not a good idea in real-time)
