...
\begin{itemize}
	\item It takes into account the impossibility to detect instantaneously a state change in a distributed system if it happens in a remote location (\define{transmission delay}) comment.
...
\begin{itemize}
	...
	\item \definebf{Time-Out}: A time-out counter is started when the state is entered. When this time-out elapses, a boolean variable \texttt{state\_name.timout} becomes true if the system is still in that state.
\end{itemize}
...
A state is \define{concerned} with a rendez-vous label is at least one of its states is labeled with it.
\\\\
