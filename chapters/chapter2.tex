...
\begin{itemize}
	...
	\item Constraints related to the computer(s) running the software. 
		...
		\end{description} 
		The goal of this strategy is to avoid that the infrastructure you selected is not able to meet the \index{timing constraints} timing constraints.
...
\begin{enumerate}
	\item real-time \index{application layer} application layer
	\item real-time operating \index{system layer} system layer
\end{enumerate}
...
\begin{enumerate}
	...
	\item Identify the services the \index{system layer} system layer must provide. (see \ref{sss:services})
...
\begin{description}
	\item[\index{Structural Model} Structural Model] Model of all the components, subsystems that can work in parallel.
	\item[\index{Behavioral Model} Behavioral Model] Model of the desired behavior e.g. state machines of components. 
\end{description}
...
\label{sss:design}
Next the \index{software architecture} software architecture can be designed by refining the \index{behavioral model} behavioral model. 
Two types of systems can be distinguished:
\begin{description}
	\item[\index{Reactive system} Reactive system] The desired behavior of the controlled device depends on the commands from the system we are designing.
		In this case the behavior of the software must be \emph{structurally similar} to that of the \indexb{controlled system}.
		The software sends commands to force the expected behavior of the controlled device.
	\item[\index{Data processing system} Data processing system] The behavior of the software must be \emph{structurally similar} to that of the \indexb{monitored system} and is driven by events occurring in that system.
\end{description}

In both cases the following process should be followed:
\begin{itemize}
	\item Model the structure of the physical system. 
	\item Make a \index{behavioral model} behavioral model of the physical system.
	\item Make a behavioral model of the software system.
\end{itemize}

The last step should give you a rough skeleton of the \index{application layer} application layer.
...
The following \index{task interactions} task interactions are possible:
\begin{itemize}
...
Each of the behavioral states of the software should be complemented with a descriptions of the \index{data processing activities} data processing activities at that state.
...
We simplify \index{task scheduling} scheduling by assuming there are only two kinds of tasks:
\begin{itemize}
...
We'll explain a simple form of \index{task synchronization} task synchronization next. 
