...
\]
R is a dimensionless value parametrized by time. The MTBF is the surface under the curve representing the evolution with respect to $R(t)$, which is the percentage of surviving items after $t$.
\begin{figure}[h!]
    \centering
...
\]
\[
N_{\text{faults}} = \frac{T_{\text{on}}}{\text{MTBF}}
\]
\[
...
A health report consists of a regular message indicating that the system is still alive, assuming the system is a \textbf{fail/stop system}.
...
The principle of doubled systems is to transfer the activity as quickly as possible from a failing main system to a working spare. The idea of multiple systems is to have enough running resources to let them \textbf{vote} by majority to produce a single output, as illustrated in Figure~\ref{tmr1}. Note that voting by majority implies having at least three modules. Moreover, such a set-up allows non \textbf{fail/stop} behavior. Indeed, if one of the systems makes an error, the other systems can correct the result through voting.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{tmr1.pdf}
    \caption{Triple Modular Redundancy set-up determining the output through majority voting.}
    \label{tmr1}
\end{figure}

Generally, we assume a system consisting of $N$ components that survives if at least $M$ components survive.
\textbf{Triple Modular Redundancy} or \textbf{TMR} is a particular case for $M=2$ and $N=3$. With TMR, only one component may fail without disrupting the end result as the other two components should correct it through majority voting.
...
\begin{itemize}
	...
	\item $R^n$ is the probability that not a single module fails in a set up of n modules.
\end{itemize}
The probability that only one out of n modules fails is
\[
...
& = R_v \cdot \left( R^3 + R^2 \cdot (1-R) \cdot 3 \right) \\
& = R_v \cdot \left( 3R^2 - 2R^3 \right)
\end{align*}
